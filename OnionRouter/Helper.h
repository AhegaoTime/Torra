...
public:
	...
	static void sendRawData(const SOCKET sc, const uint8_t* message, const uint32_t bytesNum);
	static std::string getPaddedNumber(const int num, const int digits);
	static void getRawPartFromSocket(const SOCKET& sc, uint8_t* destination, const uint32_t bytesAmount);
